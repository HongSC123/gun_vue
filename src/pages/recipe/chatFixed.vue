<template>
  ...
  axios.get(`http://localhost:8888/chatfixed?memEmail=${this.memEmail}`, {
    params: {
...
async getFindCount() {
  try {
    const response = await axios.get(`http://localhost:8888/chatfindcount?memEmail=${this.memEmail}`, {
      params: {
        boardnum: this.$route.params.id,
      }
    });
